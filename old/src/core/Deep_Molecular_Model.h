...
#ifndef __PLUMED_DEEP_MOLECULAR_MODEL_h
#define __PLUMED_DEEP_MOLECULAR_MODEL_h
...
public:
	...
	explicit Deep_Molecular_Model(const ActionOptions&ao);
	...
	virtual unsigned parameters_number() const {return 0;}
	virtual void set_parameters(const std::vector<float>&) {};
	virtual std::vector<float> get_parameters() const {return std::vector<float>(0);};
	...
	virtual dynet::Expression output(dynet::ComputationGraph& cg,const std::vector<dynet::Expression>& x,const std::vector<std::vector<unsigned>>& id){return dynet::zeros(cg,{output_dim});}
};
