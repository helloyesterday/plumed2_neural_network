...
private:
	...
	std::vector<Activation> layer_act_funs;
	MLP mlp;
public:
	...
	unsigned parameters_number() const {return mlp.parameters_number();}
	void set_parameters(const std::vector<float>& new_param) {mlp.set_parameters(new_param);}
	std::vector<float> get_parameters() const {return mlp.get_parameters();}
	...
	Neural_Network(ao)
{
	...
	for(unsigned i=0;i!=nhidden;++i)
		layer_act_funs[i]=activation_function(str_act_funs[i]);
...
void NN_MLP::build_neural_network()
{
	...
	for(unsigned i=0;i!=nhidden;++i)
	{
		mlp.append(pc,Layer(ldim,layer_dim[i],layer_act_funs[i],0));
		ldim=layer_dim[i];
	}
	mlp.append(pc,Layer(ldim,get_output_dim(),Activation::LINEAR,0));
}
