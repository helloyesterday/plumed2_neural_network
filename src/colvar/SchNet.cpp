...
dynet::Expression SchNet::calc_energy(dynet::ComputationGraph& cg,const std::vector<std::vector<dynet::Expression>>& rbf_mat,const std::vector<std::vector<unsigned>>& dis_id)
{	
	iparm=1;
	...
	for(unsigned l=0;l!=num_inlayers;++l)
	{
		std::vector<dynet::Expression> vvl=interaction(cg,vxl,rbf_mat,dis_id);
		for(unsigned iatom=0;iatom!=natoms;++iatom)
			vxl[iatom]=vxl[iatom]+vvl[iatom];
	}
	
	// read out function
	iparm_begin=iparm;
	dynet::Expression ener_sum=dynet::zeros(cg,dynet::Dim({1}));
	for(unsigned iatom=0;iatom!=natoms;++iatom)
	{
		iparm=iparm_begin;
		dynet::Expression ener=read_out(cg,vxl[iatom]);
		// sum pooling
...
std::vector<dynet::Expression> SchNet::interaction(dynet::ComputationGraph& cg,const std::vector<dynet::Expression>& vxl,const std::vector<std::vector<dynet::Expression>>& rbf_mat,const std::vector<std::vector<unsigned>>& dis_id)
{
	// atom-wise
	unsigned iparm_begin=iparm;
	std::vector<dynet::Expression> vvt;
	for(unsigned iatom=0;iatom!=natoms;++iatom)
	{
		iparm=iparm_begin;
		dynet::Expression xl=atom_wise(cg,vxl[iatom]);
		vvt.push_back(xl);
	}
	
	std::vector<dynet::Expression> vvl;
	iparm_begin=iparm;
	for(unsigned iatom=0;iatom!=rbf_mat.size();++iatom)
	{
		iparm=iparm_begin;
		// continuous-filter convolutions
		dynet::Expression cfconv=dynet::zeros(cg,{vec_dim});
		for(unsigned idis=0;idis!=rbf_mat[iatom].size();++idis)
		{
			iparm=iparm_begin;
			// filter generator
			dynet::Expression wl=filter_generator(cg,rbf_mat[iatom][idis]);
			// the element-wise multiplications
			unsigned id=dis_id[iatom][idis];
			dynet::Expression ewm=dynet::cmult(vvt[id],wl);
			cfconv=cfconv+ewm;
		}
		iparm=iparm_begin+num_dlayers;
		
		// atom-wise
		dynet::Expression xa=atom_wise(cg,cfconv);
		
		// shifted softplus
		dynet::Expression sxa=actfun(xa);
		
		// atom-wise
		dynet::Expression vl=atom_wise(cg,sxa);
